...
# ===== CONFIG =====
OWNER_IDS = [7808603044, 7507031482]  # Owner IDs list
STATE_FILE = "owners.json"

# ===== File Save/Load =====
def save_owners():
    with open(STATE_FILE, "w") as f:
        json.dump({"owners": OWNER_IDS}, f)
...
def load_owners():
    ...
    if os.path.exists(STATE_FILE):
        with open(STATE_FILE, "r") as f:
            data = json.load(f)
            OWNER_IDS = data.get("owners", OWNER_IDS)

BOT_TOKEN = "8323659076:AAE03Rej04n6J-JJP9iCwGGiL5cNS4V0AMc"
CACHED_FILE = "cached_members.json"

# အရိုးရိုး owner list
OWNERS = [7808603044]  # အလိုလို owner ID တွေ ထည့်နိုင်
OWNER_USERNAMES = ["Problem_Zenki", "RainnTheMadChief"]
...
async def list_owner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ ခွင့်ရမယ့်သူ = 7808603044 တစ်ယောက်တည်း
    if user_id != 7808603044:
        await update.message.reply_text("⛔ သင့်မှာ Owner list ကြည့်ခွင့်မရှိပါ")
        return
    
    if OWNER_IDS:
        owner_list = "\n".join([str(oid) for oid in OWNER_IDS])
        await update.message.reply_text(f"📜 Owner List:\n{owner_list}")
    else:
        await update.message.reply_text("❌ Owner list ထဲမှာ ဘယ်သူမှ မရှိပါ")
...
async def remove_owner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("❌ သုံးပုံမှန် - /remove_owner <owner_id>")
        return
    
    try:
        target_id = int(context.args[0])
    except ValueError:
        await update.message.reply_text("❌ Owner ID မမှန်ပါ")
        return

    # ✅ ဖယ်ရှားခွင့်ရတဲ့သူ = 7808603044 တစ်ယောက်တည်း
    if user_id != 7808603044:
        await update.message.reply_text("that Error Don't use is just")
        return

    # ကိုယ့်ကိုယ်ကို မဖယ်ခွင့်ပေးဘူး
    if target_id == 7808603044:
        await update.message.reply_text("⛔ ကိုယ်ပိုင် Owner ကိုမဖယ်နိုင်ပါ")
        return

    if target_id in OWNER_IDS:
        OWNER_IDS.remove(target_id)
        save_owners()
        await update.message.reply_text(f"✅ Owner ID {target_id} ကိုဖယ်ထုတ်ပြီးပါပြီ")
    else:
        await update.message.reply_text("❌ ဒီ ID က Owner ထဲမှာမရှိပါ")


async def show(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if not is_admin_or_owner(user_id):  # Make sure admin/owner only
    ...
    for handler_group in context.application.handlers.values():
        for handler in handler_group:
            if isinstance(handler, CommandHandler):
                ...
                cmds = [c for c in cmds if c not in [
                    "dead", "alive", "ping", "auto",
                    "list_owner", "remove_owner"  # <-- hide these
                ]]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    button1 = InlineKeyboardButton(
        ...
        url="https://t.me/GodRainnTechGoodGetReviews"  # ပထမ channel link
    )

    button2 = InlineKeyboardButton(
        text="Support ",
        url="https://t.me/Problem_Xz"  # ဒုတိယhannel link
    )
...
app.add_handler(CommandHandler("list_owner", list_owner))
